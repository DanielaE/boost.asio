...
void test()
{
  ...
  try
  {
    ...
    intptr_t native_socket1 = ::socket(af_inet, sock_seqpacket, 0);
...
#if !defined(BOOST_ASIO_WINDOWS_RUNTIME)
    intptr_t native_socket2 = ::socket(af_inet, sock_seqpacket, 0);
    socket1.assign(spp(af_inet, 0), native_socket2);
    intptr_t native_socket3 = ::socket(af_inet, sock_seqpacket, 0);
